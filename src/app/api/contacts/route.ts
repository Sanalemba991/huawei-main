...
const contactSchema = {
  ...
  phone: { required: true, type: 'string', minLength: 10, maxLength: 20, sanitize: true },
  company: { type: 'string', maxLength: 200, sanitize: true },
  service: { required: true, type: 'string', sanitize: true },
  ...
  try {
    ...
    if (!rateLimitResult.allowed) {
      ...
      return createSecureErrorResponse(
        `Too many contact submissions. Try again in ${retryAfterMinutes} minutes.`,
        429
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    return createSecureErrorResponse('Failed to fetch contacts', 500);
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!id) {
      return createSecureErrorResponse('Contact ID is required', 400);
    }
    ...
    if (updateData.status && !['new', 'replied', 'in_progress', 'closed'].includes(updateData.status)) {
      return createSecureErrorResponse('Invalid status value', 400);
    }

    // Validate priority values
    if (updateData.priority && !['low', 'medium', 'high'].includes(updateData.priority)) {
      return createSecureErrorResponse('Invalid priority value', 400);
    }
    ...
    if (!updatedContact) {
      return createSecureErrorResponse('Contact not found', 404);
    }
    ...
    return createSecureErrorResponse('Failed to update contact', 500);
  }
}
